...
def bellman_ford(G, source, target, weight="length"):
    """
    Bellman-Ford para MultiDiGraph.
    Retorna: (dist_total, path_nodes)
    """
    nodes = list(G.nodes())
    ...
    # Detección ciclo negativo (no debería pasar con length)
    for u, v, k, data in G.edges(keys=True, data=True):
        w = data.get(weight, None)
        if w is None:
            continue
        w = float(w)
        if dist[u] != float("inf") and dist[u] + w < dist[v]:
            raise RuntimeError("Se detectó un ciclo negativo (inesperado para length).")
...
def main():
    ...
    out_html = "cusco_10_routes_bellman_ford.html"
    out_csv = "cusco_10_routes_bellman_ford_metrics.csv"

    if not os.path.exists(graph_path):
        raise FileNotFoundError("No encontré el grafo. Ejecuta primero 01_download_cusco_city_graph.py")
    ...
    for idx, (name, (s_lat, s_lon), (e_lat, e_lon)) in enumerate(ROUTES, start=1):
        ...
        (total_meters, route_nodes), alg_ms = benchmark_once(
            lambda: bellman_ford(G, orig, dest, weight="length")
        )
